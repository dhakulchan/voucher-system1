...
class TourVoucherGeneratorV2:
    ...
    def _build_header(self, story):
        """Build Company2 header information with enhanced layout"""
        ...
        # Left side: Company info, Right side: Logo space or additional info
        header_data = [
            # Company name spanning full width
            [Paragraph(company_name, self.style_company_name), ""],
            # Address and contact details
            [Paragraph(company_address, self.style_company_details), ""],
            [Paragraph(f"Tel: {company_phone} | Mobile: {company_mobile} | Email: {company_email} |", self.style_company_details), ""],
            [Paragraph(f"Website: {company_website}", self.style_company_details), ""]
        ]
        ...
        header_table.setStyle(TableStyle([
            ...
            # Span company name across both columns for full width
            ('SPAN', (0, 0), (1, 0)),
        ]))
        
        story.append(header_table)
        story.append(Spacer(1, 15))  # Increased spacing for better separation
    ...
    def generate_tour_voucher_v2(self, booking) -> str:
        ...
        # Voucher images would go here if we had a method for them
    ...
    def generate_quote_pdf_v2(self, quote, fresh_booking=None):
        """Generate Quote PDF using WeasyPrint + Jinja2 template engine instead of ReportLab"""
        try:
            # ✅ Use WeasyPrint Quote Generator (Jinja2 + WeasyPrint)
            from services.weasyprint_quote_generator import WeasyPrintQuoteGenerator
            
            weasy_generator = WeasyPrintQuoteGenerator()
            
            self.logger.info(f"🎯 Generating Quote PDF using WeasyPrint + Jinja2 for {quote.quote_number}")
            self.logger.info(f"📋 Using booking: {fresh_booking.booking_reference if fresh_booking else 'No booking'}")
            
            if fresh_booking:
                self.logger.info(f"✅ Using FRESH booking data for more accurate quote")
            ...
            # Use WeasyPrint generator to create PDF bytes and filename
            pdf_bytes, filename = weasy_generator.generate_quote_pdf_bytes(quote, fresh_booking)
            
            self.logger.info(f"✅ Quote PDF generated using WeasyPrint: {filename} ({len(pdf_bytes)} bytes)")
            
            # Return tuple (pdf_bytes, filename) as expected by the calling code
            return pdf_bytes, filename
            
        except Exception as e:
            self.logger.error(f"❌ Error generating quote PDF with WeasyPrint: {str(e)}")
            raise

    def _build_quote_info_section(self, story, quote, booking):
        """Build quote information section"""
        # Quote details table
        quote_data = [
            ['Quote Number:', quote.quote_number, 'Quote Date:', quote.quote_date.strftime('%d/%m/%Y')],
            ['Booking Ref:', booking.booking_reference if booking else 'N/A', 'Valid Until:', quote.valid_until.strftime('%d/%m/%Y')], 
            ['Status:', quote.status.upper(), 'Total Amount:', f"THB {float(quote.total_amount):,.2f}" if quote.total_amount else "THB 0.00"]
        ]
    ...
    def _build_quote_pricing_section(self, story, quote):
        """Build quote pricing breakdown"""
        story.append(Spacer(1, 0.5*cm))
        story.append(Paragraph("QUOTATION BREAKDOWN", self.style_title))
        
        # Pricing breakdown table
        pricing_data = [
            ['Description', 'Amount (THB)'],
            ['Subtotal', f"{float(quote.subtotal):,.2f}" if quote.subtotal else "0.00"],
            ['Tax (7% VAT)', f"{float(quote.tax_amount):,.2f}" if quote.tax_amount else "0.00"],
            ['Total Amount', f"{float(quote.total_amount):,.2f}" if quote.total_amount else "0.00"]
        ]
    ...
    def _build_quote_terms_section(self, story, quote):
        """Build quote terms and conditions"""
        story.append(Spacer(1, 0.5*cm))
        story.append(Paragraph("TERMS & CONDITIONS", self.style_title))
        
        terms_text = quote.terms_conditions or """
        1. This quotation is valid for 30 days from the date of issue
        2. Prices are subject to availability and may change without notice
        3. Full payment required before service confirmation  
        4. Cancellation and refund policy applies as per company terms
        5. All services subject to third-party availability
        6. Additional charges may apply for special requests
        """
        
        story.append(Paragraph(terms_text, self.style_normal))

    def _build_qr_section_quote(self, story, qr_url):
        """Build QR code section for quote"""
        try:
            qr_image_path = self.qr.generate_qr_image(qr_url, f"quote_qr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
            qr_img = Image(qr_image_path, width=2*cm, height=2*cm)
            
            qr_data = [[qr_img, "Scan QR Code to view quote online"]]
            qr_table = Table(qr_data, colWidths=[2.5*cm, 10*cm])
            ...
            self.logger.warning(f"Could not generate QR code for quote: {e}")

    def _build_signature_section_quote(self, story):
        """Build signature section for quote acceptance"""
        story.append(Spacer(1, 1*cm))
        
        sig_data = [
            ['Customer Acceptance:', '', 'Authorized By:'],
            ['', '', ''],
