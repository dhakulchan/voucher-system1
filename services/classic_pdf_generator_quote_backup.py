...
class ClassicPDFGenerator:
    def __init__(self):
        """Initialize the Classic PDF Generator with Thai font support for Quote documents"""
        self.setup_thai_fonts()
        self.styles = self.create_styles()
        logger.info("Classic Quote PDF Generator initialized with Thai fonts")
    ...
    def create_preformatted_paragraph(self, text, style_name='ModernThai'):
    ...
    def generate_pdf(self, booking_data, products=None, output_path=None):
        ...
        def header_footer_on_page(canvas, doc):
            ...
            try:
                ...
                if os.path.exists(logo_path):
                    ...
                    # Logo position - 0.5cm from top edge
                    canvas.drawImage(logo_reader, 15*mm, A4[1] - 23*mm, width=35*mm, height=18*mm, mask='auto')
                
                # Company name - reduced font size for compact header
                canvas.setFont('Helvetica-Bold', 10)  # Reduced from 12
                canvas.setFillColor(colors.Color(0/255, 123/255, 255/255))  # Blue like sample
                canvas.drawString(55*mm, A4[1] - 10*mm, "DHAKUL CHAN TRAVEL SERVICE (THAILAND) CO.,LTD.")
                
                # Address and contact info - reduced spacing and font size
                canvas.setFont('Helvetica', 7)  # Reduced from 8
                canvas.setFillColor(colors.black)
                canvas.drawString(55*mm, A4[1] - 13*mm, "710, 716, 704, 706 Prachauthit Road, Samseenook, Huai Kwang, Bangkok 10310")
                canvas.drawString(55*mm, A4[1] - 16*mm, "Tel: +662 2744218 | +662 0266525 Fax: +662 0266525 Press 5 | Line: @dhakulchan")
                canvas.drawString(55*mm, A4[1] - 19*mm, "Website: www.dhakulchan.net | T.A.T License 11/03659")
                
                # Add "Quote / Provisional Receipt" aligned with company name
                ...
                canvas.drawString(A4[0] - 15*mm - text_width, A4[1] - 10*mm, "Quote / Provisional Receipt")
            ...
            # Create template string for "Page X of Y" - Y will be filled later
            page_text_partial = f"Page {page_num} of "
            # Use a simple approach - will be replaced with actual total on second pass
            total_pages = getattr(doc, '_total_pages', '?')
            if total_pages == '?':
                # First pass - save for later
                page_text = f"Page {page_num}"  # Temporary
            else:
                # Second pass - show full format
        ...
        # Create frame for content with gap from 0.5cm header
        # Header extends to A4[1] - 22*mm, content starts at A4[1] - 30*mm (8mm gap)
        content_top = A4[1] - 30*mm  # Small gap below header end
        frame = Frame(
        ...
        party_name = booking_data.get('customer_name') or booking_data.get('guest_name', 'TRR251022')
        status = booking_data.get('status', 'pending')
        
        # Get document title - Always Quote for this generator
        document_title = "Quote"
        title_color = '#F39C12'  # Orange for quote
        status = 'quoted'  # Force quoted status for quote generator
        
        # Generate Quote Number for paid/quoted status
        quote_number = None
        if status in ['paid', 'quoted']:
            # Generate quote number based on booking reference and date
            booking_ref = booking_data.get('booking_id', 'BK20251029G5ZX')
            # Extract date portion or use current date
            date_part = datetime.now().strftime('%m%d')
            quote_number = f"QT25{date_part}01"  # Format: QT + Year(25) + MMDD + sequence(01)
        ...
        top_row_data = [
            [
                ...
                self.create_mixed_paragraph(
                    f"<font color='#2C3E50'><b>Reference:</b></font> {booking_data.get('booking_id', 'BK20251029G5ZX')}<br/>" +
                    (f"<font color='#E67E22'><b>Quote Number:</b></font> {quote_number}<br/>" if quote_number else "") +
                    f"<font color='#8E44AD' size='9'><b>Type:</b></font> <font color='#8E44AD' size='9'>Tour Package</font>"
                )
            ]
        ]
        ...
        story.append(Spacer(1, 6))  # Reduced to minimum
        ...
        if service_detail and str(service_detail).strip() and str(service_detail).strip() not in ['No service details available', 'None', 'none', 'null']:
            # Use preformatted paragraph for proper line break handling
            try:
                service_paragraph = self.create_preformatted_paragraph(service_detail)
                ...
                story.append(Paragraph("Service detail processing error", self.styles['ModernEnglish']))
        else:
            ...
            if flight_lines:
                try:
                    for line in flight_lines:
                        flight_paragraph = self.create_mixed_paragraph(line)
                        story.append(flight_paragraph)
                    logger.info(f"✅ Added {len(flight_lines)} flight info lines to PDF")
                except Exception as e:
        ...
        if products and len(products) > 0:
            ...
            # Fallback to booking data - check multiple possible field names
            total_amount = float(booking_data.get('total_amount', 
        ...
        if guest_list and guest_list.strip():
            ...
            print(f'🔍 Quote Guest List Decoded: {repr(decoded_guest_list[:200])}...')
            
            # Check if it's plain text with line breaks or comma-separated format
                ...
                try:
                    ...
                    if isinstance(guests, list) and len(guests) > 0:
                        for i, guest in enumerate(guests, 1):
                            ...
                            formatted_guest = self.format_mixed_text(clean_guest)
                            guest_paragraph = Paragraph(f"{i}. {formatted_guest}", self.styles['ModernThai'])
                            story.append(guest_paragraph)
                    else:
                        story.append(Paragraph("—", self.styles['ModernEnglish']))
                        ...
                        for i, guest in enumerate(guests, 1):
                            ...
                            for line_num, line in enumerate(guest_lines):
                                if line.strip():  # Only process non-empty lines
                                    ...
                                    # Remove numbering - just show the name without "1." prefix
                                    guest_line = formatted_guest
                                    
                                    # Create individual paragraph for each guest line
                                    guest_style = ParagraphStyle(
        ...
        if special_request and special_request.strip() and special_request.strip() not in ['No special requests', 'No special request', 'None', 'none', '']:
            # Use preformatted paragraph for proper line break handling
            request_paragraph = self.create_preformatted_paragraph(special_request)
            story.append(request_paragraph)
            logger.info("✅ Added special request content to PDF")
        else:
        ...
        try:
            ...
            doc = BaseDocTemplate(
                output_path,
                pagesize=A4,
                topMargin=15*mm,     # Optimized for Quote documents - maximum content space
            ...
            def header_footer_on_page(canvas, doc):
                ...
                try:
                    ...
                    canvas.drawString(55*mm, A4[1] - 22*mm, "Website: www.dhakulchan.net | T.A.T License 11/03659")
                ...
                page_num = canvas.getPageNumber()
                # Use simple estimation - most booking PDFs are 2 pages
