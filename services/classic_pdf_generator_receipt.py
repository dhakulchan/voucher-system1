...
class ClassicPDFGenerator:
    ...
    def create_terms_paragraph(self, text):
        """Create paragraph for Terms & Conditions with readable font"""
        if not text:
            return Paragraph("", self.styles['ModernThai'])
        
        # Use larger font size for better readability (was 8.5, now 9.5)
        mixed_text = self.format_mixed_text(text)
        # Wrap with readable font size tag
        readable_text = f'<font size="9.5">{mixed_text}</font>'
        return Paragraph(readable_text, self.styles['ModernThai'])
    ...
    def generate_pdf(self, booking_data, products=None, output_path=None):
        ...
        def header_footer_on_page(canvas, doc):
            ...
            try:
                ...
                header_image_path = os.path.expanduser("~/templates/pdf/header-image251119.png")
                ...
                if os.path.exists(header_image_path):
                    try:
                        ...
                        # Draw logo on the left side - smaller size to match example
                        logo_x = 15*mm  # Left margin
                        logo_y = A4[1] - 30*mm  # From top
                        logo_width = 50*mm  # Smaller logo width
                        logo_height = 20*mm  # Logo height
                        ...
                        info_x = logo_x + logo_width + 8*mm  # Positioned next to logo with spacing
                        ...
                        canvas.setFont('NotoSansThai-Bold', 12)
                        canvas.setFillColor(colors.HexColor('#2C3E50'))
                        canvas.drawString(15*mm, A4[1] - 15*mm, "DHAKUL CHAN TRAVEL SERVICE (THAILAND) CO.,LTD.")
                else:
                    logger.error(f"❌ Header image not found at: {header_image_path}")
                    # Fallback: text-only header with company info
                    canvas.setFont('NotoSansThai-Bold', 12)
                    canvas.setFillColor(colors.HexColor('#1e3a8a'))
                    canvas.drawString(15*mm, A4[1] - 15*mm, "DHAKUL CHAN TRAVEL SERVICE (THAILAND) CO.,LTD.")
                    canvas.setFont('NotoSansThai-Regular', 9)
                    canvas.setFillColor(colors.black)
                    canvas.drawString(15*mm, A4[1] - 20*mm, "719, 716, 704, 706 Prachauthit Road, Samseenook, Huai Kwang, Bangkok 10310")
                ...
                canvas.drawString(A4[0] - 15*mm - text_width, A4[1] - 8*mm, "Quote / Provisional Receipt")
        ...
        # Document title - Always Quote for this generator
        ...
        story.append(Paragraph("<font color='#2C3E50' size=10><b>Payment Information / Description:</b></font>", self.styles['ModernSubtitle']))
        ...
        story.append(Spacer(1, 4))  # Space before Service Detail
        ...
        story.append(Spacer(1, 4))  # Space before Name List
        ...
        if cleaned_flight_info and str(cleaned_flight_info).strip() and str(cleaned_flight_info).strip() not in ['None', 'none', '', 'null']:
            ...
            # Filter out invalid flight info (like repeated F characters)
            valid_flight_lines = []
            for line in flight_lines:
                # Skip if line is just repeated characters
                if (line and len(line) > 3 and 
                    not all(c == line[0] for c in line) and  # Not all same character
                    line not in ['FFFFFFFFFF', 'NNNNNNNNNN', 'CCCCCCCCCC']):
                    valid_flight_lines.append(line)
            
            if valid_flight_lines:
                try:
                    for line in valid_flight_lines:
        ...
        story.append(Spacer(1, 4))  # Spacing before next section
    ...
    def generate_quote_pdf_to_buffer(self, booking_data, buffer):
        """Generate Quote PDF directly to buffer for PNG conversion"""
        try:
            logger.info(f"🎯 Generating Quote PDF to buffer for PNG conversion")
            
            # Create the story with all content
            story = self.create_story_content(booking_data)
            
            # Build PDF directly to buffer using SimpleDocTemplate
            from reportlab.platypus import SimpleDocTemplate
            ...
            logger.info("✅ Quote PDF generated successfully to buffer")
            return True
