...
class ClassicPDFGenerator:
    ...
    def generate_pdf(self, booking_data, products=None, output_path=None):
        ...
        # Get document title based on status
        if status == 'paid':
            document_title = "Quote / Provisional Receipt"
            title_color = '#27AE60'  # Green for paid
        elif status == 'quoted':
            document_title = "Quote"
            title_color = '#F39C12'  # Orange for quote
        elif status == 'vouchered':
            document_title = "Tour Voucher"
            title_color = '#8E44AD'  # Purple for voucher
        else:
            document_title = "Service Proposal"
            title_color = '#3498DB'  # Blue for default
