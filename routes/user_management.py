...
from decorators.auth_decorators import login_required, manager_required, admin_required
...
def create_user():
    ...
    if request.method == 'POST':
        ...
        if role == 'admin' and current_user.role != 'Administrator':
            errors.append('Only admin users can create admin accounts')
        ...
        try:
            ...
            user = User(
                username=username,
                email=email,
                role=role_mapping.get(role, 'Staff'),
                is_admin=(role == 'admin')
            )
...
def update_role(user_id):
    ...
    # Only admin can modify admin roles
    if (user.role == 'Administrator' or new_role == 'admin') and current_user.role != 'Administrator':
        flash('❌ Only admin users can modify admin roles', 'error')
        return redirect(url_for('user_mgmt.list_users'))
    ...
    try:
        ...
        flash(f'✅ Role updated for "{user.username}": {old_role} → {user.role}', 'success')
    except Exception as e:
        db.session.rollback()
...
def reset_password(user_id):
    ...
    if user.role == 'Administrator' and current_user.role != 'Administrator':
        flash('❌ Only admin users can reset admin passwords', 'error')
        return redirect(url_for('user_mgmt.list_users'))
...
@user_mgmt_bp.route('/<int:user_id>/delete', methods=['POST'])
@login_required
@admin_required
