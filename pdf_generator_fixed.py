...
def scrub_glyphs(text: str | None, replacement: str = "-") -> str:
    if not text:
        return ""
...
class PDFGenerator:
    ...
    def generate_tour_voucher_bytes(self, booking) -> bytes:
        """Generate tour voucher PDF bytes using WeasyPrint for better Thai font support"""
        try:
            from services.tour_voucher_weasyprint import TourVoucherWeasyPrintGenerator
            weasy_generator = TourVoucherWeasyPrintGenerator()
            return weasy_generator.generate_tour_voucher_bytes(booking)
        except Exception as e:
            self.logger.error(f"Failed to use WeasyPrint generator, falling back to legacy: {e}")
            return self._build_pdf(self._story(booking))

    def generate_tour_voucher(self, booking) -> str:
        """Generate tour voucher PDF file using WeasyPrint for better Thai font support"""
        try:
            from services.tour_voucher_weasyprint import TourVoucherWeasyPrintGenerator
            weasy_generator = TourVoucherWeasyPrintGenerator()
            full_path = weasy_generator.generate_tour_voucher(booking)
            # Extract just the filename from the full path
            return os.path.basename(full_path)
        except Exception as e:
            self.logger.error(f"Failed to use WeasyPrint generator, falling back to legacy: {e}")
            data = self.generate_tour_voucher_bytes(booking)
    ...
    def generate_tour_voucher_png(self, booking) -> str:
        """Generate tour voucher PNG file using WeasyPrint for better Thai font support"""
        try:
            from services.tour_voucher_weasyprint import TourVoucherWeasyPrintGenerator
            weasy_generator = TourVoucherWeasyPrintGenerator()
            return weasy_generator.generate_tour_voucher_png(booking)
        except Exception as e:
            self.logger.error(f"Failed to generate PNG with WeasyPrint: {e}")
            return None
